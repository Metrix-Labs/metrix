...
export function checkNodeRequirements() {
  ...
  if (!semver.satisfies(currentNodeVersion, engines.node)) {
    logger.fatal([
      ...
      `Metrix requires ${chalk.bold(chalk.green(`Node.js ${engines.node}`))}`,
  ...
  // warn if not using a LTS version
    ...
    logger.warn([
      ...
      `Metrix only supports ${chalk.bold(chalk.green('LTS versions of Node.js'))}, other versions may not be compatible.`,
    ]);
  }
}
