...
describe('Restore ', () => {
  test('Should delete all models and contentTypes', async () => {
    ...
    setGlobalStrapi(metrix);

    const { count } = await deleteRecords(metrix);
  ...
  test('Should add core store data', async () => {
    ...
    const result = await restoreConfigs(metrix, config);

    expect(metrix.db.query).toBeCalledTimes(1);
    expect(metrix.db.query).toBeCalledWith('metrix::core-store');
  ...
  test('Should add webhook data', async () => {
    ...
    const result = await restoreConfigs(metrix, config);
    expect(metrix.db.query).toBeCalledTimes(1);
    expect(metrix.db.query).toBeCalledWith('metrix::webhook');
