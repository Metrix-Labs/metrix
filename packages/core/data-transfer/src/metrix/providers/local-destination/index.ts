...
export interface ILocalStrapiDestinationProviderOptions {
  ...
  restore?: restore.IRestoreOptions; // erase data in metrix database before transfer; required if strategy is 'restore'
...
class LocalStrapiDestinationProvider implements IDestinationProvider {
  ...
  async bootstrap(diagnostics?: IDiagnosticReporter): Promise<void> {
    ...
    if (!this.metrix) {
      throw new ProviderInitializationError('Could not access local metrix');
    }
    this.metrix.db.lifecycles.disable();
    this.transaction = utils.transaction.createTransaction(this.metrix);
  }
  ...
  async #deleteFromRestoreOptions() {
    ...
    return restore.deleteRecords(this.metrix, this.options.restore);
  }
  ...
  async #deleteAllAssets(trx?: Knex.Transaction) {
    ...
    const stream: Readable = this.metrix.db
    ...
    for await (const file of stream) {
      await this.metrix.plugin('upload').provider.delete(file);
      if (file.formats) {
        for (const fileFormat of Object.values(file.formats)) {
          await this.metrix.plugin('upload').provider.delete(fileFormat);
        }
      }
    }
  ...
  getMetadata(): IMetadata {
    ...
    const strapiVersion = this.metrix.config.get<string>('info.metrix');
  ...
  createEntitiesWriteStream(): Writable {
    assertValidStrapi(this.metrix, 'Not able to import entities');
  ...
  async #handleAssetsBackup() {
    assertValidStrapi(this.metrix, 'Not able to create the assets backup');
    ...
    if (this.metrix.config.get<{ provider: string }>('plugin::upload').provider === 'local') {
      this.#reportInfo('creating assets backup directory');
      const assetsDirectory = path.join(this.metrix.dirs.static.public, 'uploads');
      const backupDirectory = path.join(
        this.metrix.dirs.static.public,
  ...
  async #removeAssetsBackup() {
    assertValidStrapi(this.metrix, 'Not able to remove Assets');
    // if we're not restoring assets, don't back them up because they won't be touched
    if (!this.#areAssetsIncluded()) {
      return;
    }
    ...
    return new Writable({
      ...
      async write(chunk: IAsset, _encoding, callback) {
        await transaction?.attach(async () => {
          ...
          const provider = metrix.config.get<{ provider: string }>('plugin::upload').provider;
          ...
          try {
            await metrix.plugin('upload').provider.uploadStream(uploadData);
            ...
            if (uploadData?.type) {
              const entry: IFile = await metrix.db.query('plugin::upload.file').findOne({
                where: { id: fileId },
              });
              if (!entry) {
              ...
              await metrix.db.query('plugin::upload.file').update({
                where: { id: entry.id },
                data: {
  ...
  async createConfigurationWriteStream(): Promise<Writable> {
    assertValidStrapi(this.metrix, 'Not able to stream Configurations');
    this.#reportInfo('creating configuration write stream');
    const { strategy } = this.options;

    if (strategy === 'restore') {
      return restore.createConfigurationWriteStream(this.metrix, this.transaction);
    }
  ...
  async createLinksWriteStream(): Promise<Writable> {
    ...
    if (strategy === 'restore') {
      return restore.createLinksWriteStream(mapID, this.metrix, this.transaction, this.onWarning);
    }
