...
describe('Component Deletion and Cleanup Test', () => {
  ...
  test('Create and delete component, then verify cleanup', async () => {
    ...
    const dbResultBeforeDeletion = await metrix.db.connection
      .select('*')
    ...
    const dbResult = await metrix.db.connection.raw(`SELECT * FROM ${contentType.pluralName}_cmps`);

    // Ensure table for the deleted component no longer exists
    const tempComponentTableExists = await metrix.db.connection.schema.hasTable(
    ...
    const keepComponentTableExists = await metrix.db.connection.schema.hasTable(
