 ...
 */
export const createLinksStream = (metrix: Core.Strapi): Readable => {
  const uids = [...Object.keys(metrix.contentTypes), ...Object.keys(metrix.components)] as string[];
  ...
  return Readable.from(
    (async function* linkGenerator(): AsyncGenerator<ILink> {
      const query = createLinkQuery(metrix);
