---
title: Event Hub
description: The event hub is a central system that processes a variety of events in a Metrix application
tags:
  - core
  - plugins
---
...
The event hub is a central system that processes a variety of events in a Metrix application. These events can be emitted from a variety of sources to trigger associated subscriber functions.
...
Events are mainly used in Metrix to power the webhooks and audit logs features. However, plugin developers can also access the event-hub using the plugin API. This means plugins can listen to events emitted by Metrix and emit new events to the event hub.
...
This design was inspired by the way Metrix handles [lifecycle hooks](https://docs.metrix.io/developer-docs/latest/development/backend-customization/models.html#lifecycle-hooks). It was chosen over the [Node.js event emitter](https://nodejs.org/api/events.html#class-eventemitter) because it provides the ability to have a single subscriber function per feature, and does not cause [memory leak concerns](https://stackoverflow.com/questions/9768444/possible-eventemitter-memory-leak-detected).
...
// Usage
metrix.eventHub.emit('some.event', { meta: 'data' });
```
...
// Add a subscriber
const unsubcribe = metrix.eventHub.subscribe((name: string, ...args: any[]) => {
...
metrix.eventHub.subscribe(subscriber);

// Use its reference to remove it
metrix.eventHub.unsubcribe(subscriber);
```
...
// Add a listener
const removeListener = metrix.eventHub.on('some.event', () => {
...
metrix.eventHub.on('some.event', listener);

// Use its reference to remove it
metrix.eventHub.off('some.event', listener);
```
...
const removeListener = metrix.eventHub.once('some.event', () => {
  // Write your listener logic here
});
...
- Performance: Metrix emits a lot of events, so you need to make sure your subscriber functions aren't too expensive to run.

## Alternatives

You may not need the event hub:

- If you want to listen to database events on a specific content type, use [lifecycle hooks](https://docs.metrix.io/developer-docs/latest/development/backend-customization/models.html#lifecycle-hooks)
- If you want to listen to database events on all content types, use a [generic database lifecycle hook](https://docs.metrix.io/developer-docs/latest/development/backend-customization/models.html#declarative-and-programmatic-usage)
- If you want to emit an event, but don't want it to be exposed to other features or plugins, create a [service](https://docs.metrix.io/developer-docs/latest/development/backend-customization/services.html#services) and call it directly instead
