...
export const createLinkQuery = (metrix: Core.Strapi, trx?: Knex.Transaction) => {
  const query = () => {
    const { connection } = metrix.db;
    ...
    async function* generateAllForAttribute(uid: string, fieldName: string): AsyncGenerator<ILink> {
      const metadata = metrix.db.metadata.get(uid);
    ...
    const insert = async (link: ILink) => {
      ...
      const metadata = metrix.db.metadata.get(left.type);
