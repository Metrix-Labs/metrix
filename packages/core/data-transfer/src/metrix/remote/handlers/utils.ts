...
const hasHttpServer = () => {
  // during server restarts, metrix may not have ever been defined at all, so we have to check it first
  return typeof metrix !== 'undefined' && !!metrix?.server?.httpServer;
};
...
const disableTimeouts = () => {
  ...
  const { httpServer } = metrix.server;
  ...
  metrix.log.info('[Data transfer] Disabling http timeouts');
};
const resetTimeouts = () => {
  ...
  metrix.log.info('[Data transfer] Restoring http timeouts');
...
export const handleWSUpgrade = (wss: WebSocketServer, ctx: Context, callback: WSCallback) => {
  ...
  wss.handleUpgrade(ctx.req, ctx.request.socket, Buffer.alloc(0), (client, request) => {
    ...
    disableTimeouts();
    metrix.db.lifecycles.disable();
    metrix.log.info('[Data transfer] Disabling lifecycle hooks');
  ...
  (options: HandlerOptions) => {
    ...
    return async (ctx: Context) => {
      const cb: WSCallback = (ws) => {
        ...
        const cannotRespondHandler = (err: unknown) => {
          ...
          try {
            ...
            metrix?.log?.error('[Data transfer] Failed to close socket on error');
          }
        };
        ...
        ws.on('close', async (...args) => {
          try {
            ...
            metrix?.log?.error('[Data transfer] Uncaught error closing connection');
            metrix?.log?.error(err);
            cannotRespondHandler(err);
          } finally {
            resetTimeouts();
            metrix.db.lifecycles.enable();
            metrix.log.info('[Data transfer] Restoring lifecycle hooks');
          }
        });
        ws.on('error', async (...args) => {
          try {
            ...
            metrix?.log?.error('[Data transfer] Uncaught error in error handling');
            metrix?.log?.error(err);
            cannotRespondHandler(err);
          }
        });
        ws.on('message', async (...args) => {
          try {
            ...
            metrix?.log?.error('[Data transfer] Uncaught error in message handling');
            metrix?.log?.error(err);
            cannotRespondHandler(err);
          }
        });
      ...
      try {
        ...
        metrix?.log?.error('[Data transfer] Error in websocket upgrade request');
        metrix?.log?.error(err);
      }
    };
  };
