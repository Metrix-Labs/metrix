...
describe('Document Service', () => {
  ...
  describe('Delete', () => {
    testInTransaction('Can delete an entire document', async () => {
      ...
      await metrix.documents(ARTICLE_UID).delete({ documentId: articleDb.documentId, locale: '*' });
    ...
    testInTransaction('Can delete a document with a component', async (trx: any) => {
      ...
      const articleDb = await findArticleDb({ title: 'Article1-Draft-EN' });
      // update article
      const updatedArticle = await metrix.documents(ARTICLE_UID).update({
      ...
      // delete article
      await metrix.documents(ARTICLE_UID).delete({
        documentId: articleDb.documentId,
        locale: 'en',
      });

      // Components should not be in the database anymore
      const compTable = metrix.db.metadata.get('article.comp').tableName;
      const dzTable = metrix.db.metadata.get('article.dz-comp').tableName;

      const comp = await metrix.db
        .getConnection(compTable)
        .where({ id: updatedArticle.comp.id })
        .transacting(trx)
        .first();
      const dz = await metrix.db
