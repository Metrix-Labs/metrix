...
import { createStrapi } from './create-metrix';
...
const command = new commander.Command('create-metrix-app')
  .version(version)
  .arguments('[directory]')
...
async function run(args: string[]): Promise<void> {
  ...
  const tmpPath = join(os.tmpdir(), `metrix${crypto.randomBytes(6).toString('hex')}`);

  const randomUUID = crypto.randomUUID();
  const uuid = (process.env.METRIX_UUID_PREFIX || '') + randomUUID;
