...
describe('Transfer', () => {
  ...
  it('exits with error when no --to or --from is provided', async () => {
    ...
    expect(
      mockDataTransfer.metrix.providers.createRemoteStrapiDestinationProvider
  ...
  describe('--to', () => {
    ...
    it('uses local Strapi source when from is not specified', async () => {
      ...
      expect(console.error).not.toHaveBeenCalled();
      expect(mockDataTransfer.metrix.providers.createLocalStrapiSourceProvider).toHaveBeenCalled();
      expect(
        mockDataTransfer.metrix.providers.createRemoteStrapiDestinationProvider
      ).toHaveBeenCalled();
    });
