...
describe('Relations interactions with disabled DP content types', () => {
  beforeAll(async () => {
    ...
    // PRODUCTS
    await metrix.db.query(PRODUCT_UID).createMany({
      data: [
    ...
    await metrix.db.query(SHOP_UID).createMany({
      data: [
    ...
    productDocuments = metrix.documents(PRODUCT_UID);
    tagDocuments = metrix.documents(TAG_UID);
    shopDocuments = metrix.documents(SHOP_UID);
  });
