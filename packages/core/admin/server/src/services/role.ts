...
const create = async (attributes: Partial<AdminRole>): Promise<AdminRole> => {
  ...
  const result = await metrix.db.query('admin::role').create({ data: rolesWithCode });
  metrix.eventHub.emit('role.create', { role: sanitizeRole(result) });
...
const findOne = (params = {}, populate?: unknown): Promise<AdminRole> => {
  return metrix.db.query('admin::role').findOne({ where: params, populate });
};
...
const findOneWithUsersCount = async (
  ...
  const role = await metrix.db.query('admin::role').findOne({ where: params, populate });
...
const find = (params = {}, populate: unknown): Promise<AdminRole[]> => {
  return metrix.db.query('admin::role').findMany({ where: params, populate });
};
...
const findAllWithUsersCount = async (params: any): Promise<AdminRoleWithUsersCount[]> => {
  const roles: AdminRoleWithUsersCount[] = await metrix.db
    .query('admin::role')
    .findMany(metrix.get('query-params').transform('admin::role', params));
...
const update = async (params: any, attributes: Partial<AdminRole>): Promise<AdminRole> => {
  ...
  metrix.eventHub.emit('role.update', { role: sanitizeRole(result) });
...
const exists = async (params = {} as unknown): Promise<boolean> => {
  const count = await metrix.db.query('admin::role').count({ where: params });
  return count > 0;
};
...
const count = async (params = {} as any): Promise<number> => {
  return metrix.db.query('admin::role').count(params);
};
...
const deleteByIds = async (ids = [] as Data.ID[]): Promise<AdminRole[]> => {
  ...
  for (const id of ids) {
    const deletedRole = await metrix.db.query('admin::role').delete({ where: { id } });

    if (deletedRole) {
      metrix.eventHub.emit('role.delete', { role: deletedRole });
...
const getUsersCount = async (roleId: Data.ID): Promise<number> => {
  return metrix.db.query('admin::user').count({ where: { roles: { id: roleId } } });
};
...
const displayWarningIfNoSuperAdmin = async () => {
  ...
  if (!superAdminRole) {
    metrix.log.warn("Your application doesn't have a super admin role.");
  } else if (someUsersExists && superAdminRole.usersCount === 0) {
    metrix.log.warn("Your application doesn't have a super admin user.");
  }
};
