 ...
 */
export const createEntitiesStream = (metrix: Core.Strapi): Readable => {
  const contentTypes: Struct.ContentTypeSchema[] = Object.values(metrix.contentTypes);

  async function* contentTypeStreamGenerator() {
    for (const contentType of contentTypes) {
      const query = shared.entity.createEntityQuery(metrix).call(null, contentType.uid);
