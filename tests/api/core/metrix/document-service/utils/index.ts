...
export const findArticleDb = async (where: any, populate: any) => {
  return metrix.db.query(ARTICLE_UID).findOne({ where, populate }) as Article | undefined;
};

export const findArticlesDb = async (where: any) => {
  return metrix.db.query(ARTICLE_UID).findMany({ where }) as Article[];
};
...
export const findAuthorDb = async (where: any) => {
  return metrix.db.query(AUTHOR_UID).findOne({ where }) as Author | undefined;
};

export const findAuthorsDb = async (where: any) => {
  return metrix.db.query(AUTHOR_UID).findMany({ where }) as Author[];
};
