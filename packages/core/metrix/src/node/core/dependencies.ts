...
/**
 * From V5 this will be imported from the package.json of `@metrix/metrix`.
 */
const PEER_DEPS = {
  react: '^18.0.0',
  'react-dom': '^18.0.0',
...
const checkRequiredDependencies = async ({
  ...
  if (install.length > 0) {
    ...
    /**
     * Re-run the same command after installation e.g. metrix build because the yarn.lock might
