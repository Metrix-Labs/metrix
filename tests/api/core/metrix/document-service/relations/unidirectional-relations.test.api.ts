...
describe('Document Service unidirectional relations', () => {
  beforeAll(async () => {
    ...
    // TAGS
    await metrix.db.query(TAG_UID).createMany({
      data: [
    ...
    // PRODUCTS
    const product = await metrix.documents(PRODUCT_UID).create({
      data: {
        name: 'Product1',
        tag: { documentId: 'Tag1' },
    ...
    // Publish tag1
    await metrix.documents(TAG_UID).publish({ documentId: 'Tag1' });
    await metrix.documents(TAG_UID).publish({ documentId: 'Tag2' });

    // Publish product
    await metrix.documents(PRODUCT_UID).publish({ documentId: product.documentId });
  });
  ...
  testInTransaction('Sync unidirectional relations on publish', async () => {
    ...
    const tag1 = await metrix.documents(TAG_UID).publish({ documentId: 'Tag1' });
    const tag1Id = tag1.entries[0].id;
    const tag2 = await metrix.documents(TAG_UID).publish({ documentId: 'Tag2' });
    const tag2Id = tag2.entries[0].id;

    const product1 = await metrix
  ...
  testInTransaction('Sync unidirectional relations on discard', async () => {
    ...
    await metrix.documents(TAG_UID).publish({ documentId: 'Tag1' });
    const tag1 = await metrix.documents(TAG_UID).discardDraft({ documentId: 'Tag1' });
    const tag1Id = tag1.entries[0].id;

    await metrix.documents(TAG_UID).publish({ documentId: 'Tag2' });
    const tag2 = await metrix.documents(TAG_UID).discardDraft({ documentId: 'Tag2' });
    const tag2Id = tag2.entries[0].id;
