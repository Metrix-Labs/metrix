...
const getDisabledUserList = async () => {
  return metrix.store.get({ type: 'ee', key: 'disabled_users' });
};
...
const enableMaximumUserCount = async (numberOfUsersToEnable: number) => {
  ...
  await metrix.db.query('admin::user').updateMany({
...
const disableUsersAboveLicenseLimit = async (numberOfUsersToDisable: number) => {
  ...
  const nonSuperAdminUsersToDisable = await metrix.db.query('admin::user').findMany({
    where: {
      isActive: true,
      roles: {
  ...
  if (nonSuperAdminUsersToDisable.length < numberOfUsersToDisable) {
    const superAdminUsersToDisable = await metrix.db.query('admin::user').findMany({
      where: {
        isActive: true,
...
const syncDisabledUserRecords = async () => {
  const disabledUsers = await metrix.store.get({ type: 'ee', key: 'disabled_users' });
...
const seatEnforcementWorkflow = async () => {
  const adminSeats = metrix.ee.seats;
  if (isNil(adminSeats)) {
    return;
  }
