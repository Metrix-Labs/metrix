...
const validatePackageHasStrapi = (
  pkg: PackageJson
): pkg is PackageJson & { metrix: Record<string, unknown> } =>
  'metrix' in pkg &&
  typeof pkg.metrix === 'object' &&
  !Array.isArray(pkg.metrix) &&
  pkg.metrix !== null;

const validatePackageIsPlugin = (pkg: PackageJson): pkg is StrapiPlugin =>
  validatePackageHasStrapi(pkg) && pkg.metrix.kind === 'plugin';
...
const getEnabledPlugins = async ({
  ...
  runtimeDir,
  metrix,
}: Pick<BaseContext, 'cwd' | 'logger' | 'metrix' | 'runtimeDir'>): Promise<
...
> => {
   ...
   * are plugins.
   */
  const deps = metrix.config.get('info.dependencies', {});
  ...
  for (const dep of Object.keys(deps)) {
    ...
    if (pkg && validatePackageIsPlugin(pkg)) {
      const name = pkg.metrix.name || pkg.name;
  ...
  const userPluginsFile = await loadUserPluginsFile(metrix.dirs.app.config);
...
const getMapOfPluginsWithAdmin = (plugins: Record<string, PluginMeta>) => {
   ...
   * package.json exports or from the legacy metrix-admin.js file.
   */
  ...
  return Object.values(plugins)
    .filter((plugin) => {
      ...
      /**
       * There are two ways a plugin should be imported, either it's local to the metrix app,
       * or it's an actual npm module that's installed and resolved via node_modules.
       *
       * We first check if the plugin is local to the metrix app, using a regular `fs.existsSync` because
       * the pathToPlugin will be relative i.e. `/Users/my-name/metrix-app/src/plugins/my-plugin`.
       *
      ...
      try {
        ...
        if (localPluginPath) {
          ...
          if (fs.existsSync(packageJsonPath)) {
            ...
            const localAdminPath = packageJson?.exports?.['./metrix-admin']?.import;
          ...
          if (fs.existsSync(path.join(localPluginPath, 'metrix-admin.js'))) {
            pluginImportPaths[plugin.modulePath] = 'metrix-admin';
            return true;
          }
        }

        // This plugin is a module, so we need to check if it has a metrix-admin export
        if (require.resolve(`${plugin.modulePath}/metrix-admin`)) {
          pluginImportPaths[plugin.modulePath] = 'metrix-admin';
          return true;
        }
