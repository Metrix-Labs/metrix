...
const readPackageJSON = async (path: string) => {
  try {
    ...
    const uuid = packageObj.metrix ? packageObj.metrix.uuid : null;
    const installId = packageObj.metrix ? packageObj.metrix.installId : null;
...
const action = async () => {
  ...
  if ((packageObj.metrix && packageObj.metrix.telemetryDisabled) || !uuid) {
  ...
  if (!write) {
    console.log(
      `${chalk.yellow(
        'Warning'
      )}: There has been an error, please set "telemetryDisabled": true in the "metrix" object of your package.json manually.`
    );
    process.exit(0);
  }
