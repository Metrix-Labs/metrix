...
import { DefaultDocument } from '@metrix/admin/_internal';
...
const getEntryModule = (ctx: BuildContext): string => {
  ...
  return outdent`
         ...
         */
        ${pluginsImport}
        import { renderAdmin } from "@metrix/metrix/admin"
        ...
        renderAdmin(
          document.getElementById("metrix"),
          {
...
const writeStaticClientFiles = async (ctx: BuildContext) => {
  ...
  /**
   * For everything to work effectively we create a client folder in `.metrix` at the cwd level.
