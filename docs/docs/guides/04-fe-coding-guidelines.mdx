...
The FE of Metrix aims to be a domain driven application such as we heavily utilise the concept of
nested routes:
...
Prefer named exports over default exports. Why? Because Metrix is a _very_ large codebase. If you
...
import { useFetchClient } from '@metrix/helper-plugin'
...
```jsx
jest.mock('@metrix/helper-plugin', () => ({
  ...jest.requireActual('@metrix/helper-plugin'),
