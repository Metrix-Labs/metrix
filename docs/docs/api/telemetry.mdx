...
The telemetry service is responsible for collecting and sending anonymous usage data to Metrix. This service is disabled by default, but can be enabled or disabled via configuration.

## Usage Information

The collected usage data is used to help Metrix improve the product by identifying areas of improvement, tracking feature adoption, and measuring performance. You can learn more about the usage data that is collected by visiting the following link:

https://docs.metrix.io/developer-docs/latest/getting-started/usage-information.html

## Module: Telemetry Service

### createTelemetryInstance()

- metrix: [<Type>Metrix</Type>](Metrix) - A metrix instance.
...
const telemetry = createTelemetryInstance(metrix);
```
...
Examples of event properties in Metrix include model, containsRelationalFields, displayedFields, kind. These properties are specific to the event and are used to provide additional context about what happened.
