...
const updateArticle = async (params: Modules.Documents.ServiceParams['update']) => {
  return metrix.documents(ARTICLE_UID).update({ ...params });
};
...
describe('Document Service', () => {
  ...
  describe('Update', () => {
    ...
    testInTransaction(
      ...
      async () => {
        // Covers issue https://github.com/metrix/metrix/issues/21594

        const MIXED_CONTENT_UID = 'api::mixed-content.mixed-content';

        // Create a document with both localized and non-localized fields
        const originalDoc = await metrix.documents(MIXED_CONTENT_UID).create({
          data: {
        ...
        const updatedDoc = await metrix.documents(MIXED_CONTENT_UID).update({
        ...
        const originalEnDoc = await metrix.documents(MIXED_CONTENT_UID).findOne({
          documentId: originalDoc.documentId,
          locale: 'en',
        });
