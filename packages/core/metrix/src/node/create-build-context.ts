...
const createBuildContext = async <TOptions extends BaseOptions>({
  ...
  /**
   * If you make a new metrix instance when one already exists,
  ...
  const env = getStrapiAdminEnvVars({
    ...
    STRAPI_ANALYTICS_URL: process.env.METRIX_ANALYTICS_URL || 'https://analytics.metrix.io',
  });
  ...
  const runtimeDir = path.join(cwd, '.metrix', 'client');
  const entry = path.relative(cwd, path.join(runtimeDir, 'app.js'));

  const plugins = await getEnabledPlugins({ cwd, logger, runtimeDir, metrix: strapiInstance });
