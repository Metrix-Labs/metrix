#!/usr/bin/env node
/*
 * Rename all files and directories containing "strapi" to "metrix".
 *
 * Usage:
 *   node scripts/rename/rename-strapi-to-metrix.js --dry   # preview
 *   node scripts/rename/rename-strapi-to-metrix.js         # apply
 */
...
/** Compute next path by replacing all occurrences of "strapi" with "metrix" */
function computeNextPath(relPath) {
  return relPath.replace(/strapi/g, 'metrix');
}
...
async function main() {
  ...
  const dirs = discovered.filter((e) => e.isDir && e.rel.includes('strapi'));
  ...
  // Files next
  const files = discovered.filter((e) => !e.isDir && e.rel.includes('strapi'));
