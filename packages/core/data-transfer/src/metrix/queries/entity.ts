...
import type { Core, UID, Data, Struct } from '@metrixlabs/types';
...
const createEntityQuery = (metrix: Core.Strapi): any => {
  const components = {
  ...
  const query = (uid: UID.Schema) => {
    const create = async <T extends { data: U }, U extends object>(params: T) => {
      ...
      return metrix.db.query(uid).create({ ...params, data: sanitizedData });
    };
    ...
    const createMany = async <T extends { data: U[] }, U extends object>(params: T) => {
      return (
        Promise.resolve(params.data)
          ...
          // Execute a metrix db createMany query with all the entities + their created components
          .then((data) => metrix.db.query(uid).createMany({ ...params, data }))
      );
    };

    const deleteMany = async <T extends object>(params?: T) => {
      const entitiesToDelete = await metrix.db.query(uid).findMany(params ?? {});
      ...
      const deletedEntities = await metrix.db.query(uid).deleteMany(params);
      ...
      for (const [key, attribute] of entries) {
        ...
        if (attribute.type === 'dynamiczone') {
          ...
          for (const componentUID of componentsUID) {
            const component = metrix.getModel(componentUID);
            const subPopulate = getDeepPopulateComponentLikeQuery(component, params);
