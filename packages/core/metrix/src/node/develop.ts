import * as tsUtils from '@metrix/typescript-utils';
...
const develop = async ({
  ...
  if (cluster.isWorker) {
     ...
     * as a metrix middleware.
     */
    let bundleWatcher: WebpackWatcher | ViteWatcher | undefined;

    const strapiInstance = await metrix.load();
    ...
    if (tsconfig?.config || metrix.config.get('typescript.autogenerate') !== false) {
      timer.start('generatingTS');
      const generatingTsSpinner = logger.spinner(`Generating types`).start();

      await tsUtils.generators.generate({
        metrix: strapiInstance,
        pwd: cwd,
        rootDir: undefined,
    ...
    process.on('message', async (message) => {
      switch (message) {
        case 'kill': {
          logger.debug(
            'child process has the kill message, destroying the metrix instance and sending the killed process message'
          );
          await watcher.close();
