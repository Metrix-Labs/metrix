...
const watch = async (ctx: BuildContext): Promise<ViteWatcher> => {
  ...
  if (typeof hmrConfig === 'object' && hmrConfig.server === hmrServer) {
    ...
    metrix.server.httpServer.on('listening', async () => {
  ...
  const serveAdmin: Core.MiddlewareHandler = async (koaCtx, next) => {
    ...
    let template = await fs.readFile(path.relative(ctx.cwd, '.metrix/client/index.html'), 'utf-8');
  ...
  ctx.metrix.server.router.get(adminRoute, serveAdmin);
  ctx.metrix.server.router.use(adminRoute, viteMiddlewares);
