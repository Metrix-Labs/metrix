...
export const deleteRecords = async (metrix: Core.Strapi, options: IRestoreOptions) => {
  const entities = await deleteEntitiesRecords(metrix, options);
  const configuration = await deleteConfigurationRecords(metrix, options);
...
const deleteEntitiesRecords = async (
  ...
  const models = metrix.get('models').get() as Model[];
  const contentTypes = Object.values(metrix.contentTypes) as Struct.ContentTypeSchema[];
  ...
  const contentTypeQuery = queries.entity.createEntityQuery(metrix);
  ...
  const modelsPromises = modelsToClear.map(async (uid) => {
    const result = await metrix.db.query(uid).deleteMany({});
...
const deleteConfigurationRecords = async (
  ...
  if (coreStore) {
    models.push('metrix::core-store');
  }

  if (webhook) {
    models.push('metrix::webhook');
  }
