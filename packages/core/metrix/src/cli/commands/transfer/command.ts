  ...
  return (
    createCommand('transfer')
      ...
      .hook('preAction', async (thisCommand) => {
        ...
        const hasEnvUrl = process.env.METRIX_TRANSFER_URL;
        const hasEnvToken = process.env.METRIX_TRANSFER_TOKEN;

        const logDocumentation = () => {
          console.info(
            'ℹ️  Data transfer documentation: https://docs.metrix.io/dev-docs/data-management/transfer'
          );
        };
        ...
        const determineUrl = async (direction: 'from' | 'to') => {
          ...
          if (process.env.METRIX_TRANSFER_URL) {
            return new URL(process.env.METRIX_TRANSFER_URL);
          }
          ...
          const answer = await inquirer.prompt([
            {
              ...
              default: process.env.METRIX_TRANSFER_URL,
        ...
        const determineToken = async (direction: 'from' | 'to') => {
          ...
          if (process.env.METRIX_TRANSFER_TOKEN) {
            return process.env.METRIX_TRANSFER_TOKEN;
          }
          ...
          const answer = await inquirer.prompt([
            {
              ...
              default: process.env.METRIX_TRANSFER_TOKEN,
              validate(input: string) {
                if (!input?.length) {
